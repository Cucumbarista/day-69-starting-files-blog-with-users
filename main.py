...
def register():
    ...
    if form.validate_on_submit():
        result = db.session.execute(db.select(BlogUser).where(BlogUser.email == request.form["email"]))
        check_user = result.scalar()
        if check_user:
            flash("This Email-Address is already registered. Login instead.")
...
def login():
    ...
    if form.validate_on_submit():
        ...
        user = result.scalar()
        if not user:
            flash("Your Email-Address is not registered.")
...
def show_post(post_id):
    ...
    if form.validate_on_submit():
        if not current_user.is_authenticated:
            flash("Only members can leave comments. Please log in.")
            return redirect(url_for("login"))
        new_comment = Comment(
            text=form.body.data,
